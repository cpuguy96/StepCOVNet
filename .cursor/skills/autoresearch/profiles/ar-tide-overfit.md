...
| **Teacher ordered vs aux F1**       | `ordered=634/634` but `event_f1&lt;1` (Hungarian vs raw chart)                                         | Ordered vs `target_times` is the teacher gate; event F1 is aux only — free-run still runs                            |
