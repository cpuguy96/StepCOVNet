...
**Related:** [discussion notes](DISCUSSION_NOTES.md) · [experiment log](EXPERIMENT_LOG.md) · [paper outline](PAPER_OUTLINE.md) · [decisions checklist](DECISIONS_CHECKLIST.md) · [planning doc](../onset_output_targets_planning.md) · [tide shape walkthrough](TIDE_SHAPE_WALKTHROUGH.md)
...
audio
  ...
  → [MODEL]   frontend (if not pre-cached) → temporal encoder → head
              → raw outputs: K query slots (time, confidence) OR dense frame probs
  → [POST]    confidence threshold → sort → min-gap / NMS → sparse onset list
...
| **Dense (baseline)**       | Per-frame onset probability vector            | U-Net frame classifier                  |

**Swappable:** encoder depth/capacity, `num_queries` K, time head (learnable deltas vs fixed refs), formulation (query slots vs dense frames vs future seq2seq).
...
| Step      | Event path (inference)                   | Notes                                                                  |
| --------- | ---------------------------------------- | ---------------------------------------------------------------------- |
| Threshold | Keep slots with `confidence ≥ threshold` | Default 0.5; cheap to sweep on a fixed checkpoint                      |
| Sort      | By time                                  |                                                                        |
| Min-gap   | Collapse predictions closer than ~50 ms  | Matches dense-path spirit; **not always applied in val metrics today** |
...
| Component       | Event path (current)                                     | Aligns with eval?                                   |
| --------------- | -------------------------------------------------------- | --------------------------------------------------- |
| Assignment      | Hungarian L1 on raw time error (`assign_onset_pairs_l1`) | Pairing semantics aligned; eval adds tolerance gate |
| Time loss       | L1 + L2 + beyond-tolerance penalty on matched pairs      |                                                     |
| Confidence loss | BCE toward 1/0 from per-pair time error vs tolerance     |                                                     |
| Val metrics     | `event_onset_f1`, `event_onset_f1_mingap`                | Checkpoint on raw F1 (A1)                           |
...
Use dense MERT as the strongest in-repo baseline when judging whether the event formulation is worth the complexity.
...
- PRE in cache vs in Keras graph (e2e learnability vs speed)?
- Should primary val metric apply inference POST (min-gap) or raw K slots?
- Single config schema `{pre, model, post, metric}` for all ablations?
- Can query-slot MODEL memorize single-song overfit without shortcuts, or is formulation the ceiling?

Track decisions in [DECISIONS_CHECKLIST.md](DECISIONS_CHECKLIST.md).
