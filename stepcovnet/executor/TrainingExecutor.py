...
class TrainingExecutor(AbstractExecutor):
    ...
    def execute(self, input_data: TrainingInput):
        ...
        weights = self.stepcovnet_model.model.get_weights() if hyperparameters.retrain else None

        self.stepcovnet_model.model.compile(loss=hyperparameters.loss,
    ...
    def train(self, input_data, callbacks):
        ...
        print("\nStarting training...")
        history = self.stepcovnet_model.model.fit(x=input_data.train_generator,
    ...
    def retrain(self, input_data, saved_original_weights, epochs, callbacks):
        print("Training on %d samples (%d songs)" % (input_data.all_feature_generator.num_samples,
                                                     len(input_data.all_feature_generator.train_indexes)))
        print("\nStarting retraining...")
        self.stepcovnet_model.model.set_weights(saved_original_weights)
        history = self.stepcovnet_model.model.fit(x=input_data.all_generator,
                                                  epochs=epochs,
