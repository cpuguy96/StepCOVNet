...
def feature_label_weights_saver(path_output, filename, feature, label, sample_weights):
    ...
    filename_feature_all = join(path_output, 'feature_' + 'jingju.h5')
    h5f = h5py.File(filename_feature_all, 'w')
    h5f.create_dataset('feature_all', data=feature)
    h5f.close()

    pickle.dump(label, open(join(path_output, 'labels_' + 'jingju.pkl'), 'wb'), protocol=2)

    pickle.dump(sample_weights, open(join(path_output, 'sample_weights_' + 'jingju.pkl'), 'wb'), protocol=2)
...
def dump_feature_label_sample_weights_onset_phrase_bock(audio_path,
    ...
    scaler.fit(log_mel_line_all)
    pickle.dump(scaler, open(join(path_output, 'jingju.pkl'), 'wb'), protocol=2)
